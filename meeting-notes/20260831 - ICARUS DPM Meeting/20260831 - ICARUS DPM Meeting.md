...
## ago 31, 2026 09:00 GMT-5

## Attendees

Alessandro Maria Ricci, Manuel Dallolio, Matthew Siden, Vito Di Benedetto, Fatima Abd Alrahman, Tracy Usher, Giuseppe Cerati, Dihan Senadheera, Saurav Saha, Giovanni Chiello, Gianmarco Cucciniello
...
| **ICARUS dCache Persistent Usage per User** Total is 114 TiB: [link](https://fifemon.fnal.gov/monitor/d/000000175/dcache-persistent-usage-by-vo?orgId=1&var-VO=icarus), Used space: 108 TiB (94.2%) |  **SBN dCache Persistent Usage per User** Total is 426 TiB: [link](https://fifemon.fnal.gov/monitor/d/000000175/dcache-persistent-usage-by-vo?orgId=1&var-VO=sbn), Used space: 155 TiB (36.5%) |
...
* Request 87: remove missing files and relaunch with the draining datasets  
* Request 96: testing POMS automatic submission  
* Request 100: Help Fatima with the cfg  
* Requests 101-102: debugging production workflow  
* Requests 103-104: Manuel is running g4-stage0 for 103, Manuel is generating the gen files for 104\.
...
* GiovanniVito Remove the old keepup datasets from SBNDataPool and scratch (icaruspro is using 900 TB and icarus group is using 1.6 PB):  
  * Stage0 in Scratch  
  * Calibntuples in SBNDataPool  
  * Caf and flatcaf in SBNDataPool
...
* Saurav Transfer of BNB stage1 run2 to tape:  
  ...
  * Icaruspro\_2024\_Run2\_production\_Reproc\_Run2\_v09\_89\_01\_01p03\_offbeambnbmajority\_stage1 (70 TB) \-\> Saurav is fixing the missing metadata  
  * icaruspro\_production\_v09\_89\_01\_01\_2024A\_ICARUS\_MC\_Sys\_NuCos\_2024A\_MC\_Sys\_NuCos\_CV\_2ndV\_stage1 (51 TB) \-\> The log file is missing

  Some files remained on disk because they do not have metadata.


* Saurav Transfer of BNB Run 2 systematics sample to tape:  
...
* SauravDihan Transfer of stage1 of Run3 NuMI offbeam from scratch to tape: production\_data\_2026\_ICARUS\_Run3\_NuMI\_RHC\_Run3\_Data\_Offbeamnumimajority\_v09\_89\_01\_02p02\_stage1 \-\> started.  
  * When the transfer is complete, remove the old sam location  
  * Launch a validation of the dataset without \-p option \-\> % of the files that are still in tape  
  * If the statistics are enough \=\> relaunch the previous command with \-p option
...
* These are the files dCache reported had file transfer issues, so the could be corrupted. Paths in the file are using the dCache namespace, so the paths need to be mapped like this /pnfs/fs/usr/ \-\> /pnfs/  
...
* Alessandro Clean icaruspro home folder (131 GB):  
  * All 2022 folders  
* ICARUS Persistent:  
  * Dihan Contact SPINE to remove some of 12.55 TiB on ICARUS Persistent  
  * Giuseppe Contact Cristian Farnese asking to clean his folder in ICARUS Persistent  
...
* PromitaFatima update and clean the web site  
* Fatima Clean SBNData2Pool  
  * We have a strong discrepancy between SAM and dust \-\> to be fixed
...
* Vito:  
  ...
  * SDumont in Brazil (LNCC): setup ongoing \-\> increase the number of jobs can be submitted