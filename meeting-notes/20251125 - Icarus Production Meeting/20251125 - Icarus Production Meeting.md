## Nov 25, 2025 9:00 AM CST | ICARUS Production Meeting

### Attendees

![](images/image3.png)

### Monitoring resource usage

| User Grid Usage History of the *Running Jobs by User* for the last 7 days: [link](https://fifemon.fnal.gov/monitor/d/000000053/experiment-batch-details?orgId=1&viewPanel=9&from=now-7d&to=now&var-experiment=icarus&var-pool=dune-global&var-pool=fifebatch) | User Job Efficiency History of the User Job Efficiency for the last 7 days: [link](https://fifemon.fnal.gov/monitor/d/000000022/experiment-efficiency-details?from=now-7d&to=now&var-experiment=icarus&var-pool=dune-global&var-pool=fifebatch&orgId=1&viewPanel=2) |
| ----- | ----- |
| ![](images/image1.png) | ![](images/image5.png) |
| **Icaruspro Jobs Exit Code** History of the icaruspro job exit code for the last 7 days: [link](https://landscape.fnal.gov/kibana/app/kibana#/dashboard/ba047b90-b8ca-11e7-989a-91951b87e80a?_g=\(refreshInterval:\(pause:!t,value:0\),time:\(from:now-4d,mode:relative,to:now\)\)&_a=\(description:'View%20jobs%20exit%20code,%20where%20they%20ran,%20and%20logs',filters:!\(\('$state':\(store:appState\),meta:\(alias:!n,disabled:!f,index:'fifebatch-history-*',key:pool,negate:!f,params:\(query:fifebatch,type:phrase\),type:phrase,value:fifebatch\),query:\(match:\(pool:\(query:fifebatch,type:phrase\)\)\)\),\('$state':\(store:appState\),meta:\(alias:!n,disabled:!f,index:'fifebatch-history-*',key:User,negate:!f,params:\(query:'icaruspro@fnal.gov',type:phrase\),type:phrase,value:'icaruspro@fnal.gov'\),query:\(match:\(User:\(query:'icaruspro@fnal.gov',type:phrase\)\)\)\),\('$state':\(store:appState\),meta:\(alias:!n,disabled:!f,index:'fifebatch-history-*',key:Jobsub_Group,negate:!f,params:\(query:icarus,type:phrase\),type:phrase,value:icarus\),query:\(match:\(Jobsub_Group:\(query:icarus,type:phrase\)\)\)\)\),fullScreenMode:!f,options:\(darkTheme:!f\),panels:!\(\(embeddableConfig:\(vis:\(colors:\(Cancelled:%23967302,Fail:%23BF1B00,Success:%23629E51\),legendOpen:!t\)\),gridData:\(h:15,i:'1',w:40,x:0,y:0\),id:'2f40f420-b8ca-11e7-989a-91951b87e80a',panelIndex:'1',type:visualization,version:'6.8.23'\),\(gridData:\(h:10,i:'2',w:24,x:24,y:15\),id:'569cca30-b8ca-11e7-989a-91951b87e80a',panelIndex:'2',type:visualization,version:'6.8.23'\),\(gridData:\(h:10,i:'3',w:24,x:0,y:15\),id:'65759a00-b8ca-11e7-989a-91951b87e80a',panelIndex:'3',type:visualization,version:'6.8.23'\),\(embeddableConfig:\(columns:!\(JobsubJobId,Owner,ExitCode,ExitSignal,MATCH_GLIDEIN_Site,MachineAttrMachine0,stdout,stderr\),sort:!\('@timestamp',desc\)\),gridData:\(h:30,i:'4',w:48,x:0,y:25\),id:'7e94c3c0-b8cb-11e7-989a-91951b87e80a',panelIndex:'4',type:search,version:'6.8.23'\),\(gridData:\(h:15,i:'5',w:8,x:40,y:0\),id:AWZpvkXbLj3wKbt0N_Vp,panelIndex:'5',type:visualization,version:'6.8.23'\)\),query:\(language:lucene,query:\(match_all:\(\)\)\),timeRestore:!f,title:'Fifebatch%20History',viewMode:view\)) | **SBN Data Pools** [link](https://fifemon.fnal.gov/monitor/d/rflbgV-iz/dcache-by-poolgroup?orgId=1&var-PoolGroup=SbnData2Pools&from=now-3h&to=now&refresh=5m) |
| ![](images/image4.png) | ![](images/image7.png) |
| Dcache Persistent Usage per user Total is 114 TB: [link](https://fifemon.fnal.gov/monitor/d/000000175/dcache-persistent-usage-by-vo?orgId=1&var-VO=icarus) |  |
| ![](images/image6.png) |  |

### Production requests

| 2025 Not-Completed Production Requests |
| ----- |
| ![](images/image2.png) |

Link to [spreadsheet](https://docs.google.com/spreadsheets/d/1ffBp475tEzlRilFs7xLhbevSZHjsuk1Dm5FGFIPWsFM/edit?gid=1567393491#gid=1567393491)  
Link to [github project](https://github.com/orgs/SBNSoftware/projects/49)

### Active Campaigns in POMS

| Campaign Name | ID | Creator | Note |
| :---- | :---- | :---- | :---- |
| 2025A\_ICARUS\_SPINE\_detsim\_CV\_NERSC | 10442 | amricci | \#110 |
| 2025A\_ICARUS\_Overlays\_BNB\_RUN2 | 10165 | manueld | \#75, \#86 |
| 2025\_ICARUS\_BNB\_Run2\_v10\_SBN | 10070 | manueld | ?? |
| ICARUS\_keepup | 9442 | icaro | keepup |

Link [here](https://pomsgpvm02.fnal.gov/poms/show_campaigns/icarus/production)

### Notes

* 

### Requests

* Assigned:  
  * Request \#61 \[Thomas\]: \[RUN4\]   
    1. MC: The main obstacle is related to the detsim which is consuming a considerable amount of memory. Checks are presently underway to either decrease the memory usage or identify an alternative solution that will allow it to run at Polaris.   
    2. Data: Reprocessing of Stage1 following the application of calibration not started yet; bnbmajority and the offbeambnbmajority have been processed utilizing the new DNNROI framework. However, the minbias has specifically not been processed for Run 4  
  * Request \#75 \[Manuel/Ivan\]:  
    1. Everything concerning normal overlays has been stopped  
  * Request \#86 \[Manuel/Ivan\]:   
    1. See \#75. Everything concerning normal overlays has been stopped  
  * Request \#100 \[Ivan\]: 	  
    1. Done. Info missing in the db  
  * Request \#108 \[Promita\]:  
    1. Currently running and is more than 50% complete. Stage0 is already done. Waiting for Stage1 to be completed  
  * Request \#109 \[Manuel\]:  
    1. Stage0 is done; waiting for stage1 fclt  
  * Request \#110, \#111 \[Alessandro\]:  
    1. \#111: Running and are nearing completion, with only the last 20% remaining. Although the final jobs were recently submitted, they failed because the new files produced by Gray utilized the name "G4" for Stage G4, which differed from the old files named "Geant4". Alessandro has since addressed this issue by changing the configuration file and launching a test. It is expected that if this test is successful, the final part of the campaign will commence the following day, and the overall campaign is anticipated to be finished within three to five days.  
    2. \#110: campaign has been started, and approximately half of the events have been generated so  
  * Request \#112 \[Promita\]:  
    1. Paused; Waiting for stage1 fcl  
  * Request \#113 \[Antonio\]:  
    1. Running test or full production  
    2. CAF files with calibrations and updated file format, Run 2 off-beam data  
  * Request \#114 \[Ivan\]:  
    1. The campaign is still running offsite exclusively, which was done under requests from Gray  
  * Request \#114 \[Fatima\]:  
    1. Assigned to Fatima

### Action Items and Open issue

* Link to [action items](https://github.com/orgs/SBNSoftware/projects/32)  
* \[Promita\] Datasample deletion campaign: still ongoing. The campaign was intended to delete various datasets, including compressed mini-data which was not backed out in validation. Promita has been actively seeking feedback from users of these datasets, including individuals who use the compressed mini-data and other datasets, as they require more time to grant the "green flag" for deletion. Once this internal feedback process is complete, Promita plans to send an official email to the collaboration, noting that sending the email directly to the collaboration at the outset would have created confusion rather than confirming the current plan  
* \[Matteo\] *icaruscode* reproducibility: ongoing. Here [details](https://shortbaseline.slack.com/docs/T7P7C3UAK/F0A0K0PRR16)  
* Michael Carrigan disk usage: is it ok?

### CNAF

* **RUN3 Processing**: The Run 3 processing at CNAF is currently stopped for the on-beam data, having reached 95.5% completion. The investigation is focused on the remaining 4% of files that are failing due to a non-resource-related issue, specifically a fatal error involving an event auxiliary mismatch where different numbers of entries are found in branches (e.g., auxiliary has 23 entries instead of 24), causing the processing step (apparently Stage1) to crash. While the on-beam processing is paused to resolve this, the team is simultaneously preparing to process the off-beam data. This preparation involves moving older production data or items to tape to clear disk space, as the utilized storage was near the 2 petabyte pledge (at 1.94 petabytes). Approximately half a petabyte has been moved so far, and once the moves are complete, deletions will follow to recover space, allowing the off-beam processing to commence. There is an ongoing discussion about which off-beam datasets should be processed, with the current understanding that the minbias trigger has not been processed for Run 2 or Run 4, and therefore investigating whether processing the minbias first for calibration, before the majority, is necessary is key to avoiding wasted effort.

### Keepup

* \[Ivan\]:   
  * NTR

### Infrastructure

* \[Giuseppe\]:

### Software

* \[Tracy\]:   
  * NTR

### Computing

* \[Vito\]: Significant change in the resilient file storage policy impacting production activities. Approximately a month ago, the resilient storage system changed such that files that have existed in resilient for longer than one month will be automatically deleted, regardless of whether they have had recent access, as the mechanism to check for recent access is inadequate. This policy affects the production area, which primarily uses resilient storage for tarballs and scripts essential for production. To handle this, one current workaround is to copy the tarballs at least once a month to ensure a new version exists and avoids deletion. As a potential long-term solution, Vito mentioned a proposed feature in job submission software that would allow jobs to check out a specific branch or revision from a GitHub repository and deploy its content directly into the job's working area. This feature would be useful for managing production scripts, potentially in a dedicated GitHub repository, avoiding the need to worry about files being deleted from resilient. Vito indicated that if this feature is deemed useful, he can push for its inclusion in upcoming job submission releases, which are expected early next year